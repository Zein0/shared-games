...
function GamesList() {
    ...
    const fetchGames = async () => {
      try {
        const [psResponse, xboxResponse] = await Promise.all([
          ...
          fetch(`${apiBaseUrl}/api/games/xbox`)
        ]);
        ...
        const xboxGames = xboxData.games.map(game => ({ ...game, consoles: ['Xbox'] }));

        const combinedGames = [...xboxGames, ...psGames];

        const mergedGames = combinedGames.reduce((acc, current) => {
          const existingGame = acc.find(game => game.title === current.title);
          if (existingGame) {
            existingGame.tag = Array.from(new Set([...existingGame.tag, ...current.tag]));
            existingGame.consoles = Array.from(new Set([...existingGame.consoles, ...current.consoles]));
          } else {
            acc.push(current);
          }
          return acc;
  ...
  .filter((game) =>game.consoles.includes('PS') && game.consoles.includes('Xbox'))
  const totalPages = Math.ceil(commonGames.length / itemsPerPage);
  const currentGames = commonGames.slice(startIndex, endIndex);
              ...
              <CardContent className="p-0 flex flex-col min-h-full">
                <div className="relative h-[300px] overflow-hidden rounded-t-lg">
                  <img
                    ...
                    height="300"
                    src={showImage ? game.image : fallBack}
                    key={game.image}
                    width="400" />
                </div>
